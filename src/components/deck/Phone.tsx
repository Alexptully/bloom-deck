...
export function GardenWall({ full, size = 420, className = "" }: { full: boolean; size?: number; className?: string }) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-card shadow-card ${className}`} style={{ width: size, height: size * 1080 / 1020 }}>
      <Image src={asset("/renders/garden-empty.jpg")} alt="Projected garden clearing with no flowers yet" width={1020} height={1080} priority style={{ width: "100%", height: "100%" }} />
      <motion.div
      ...
      >
        <Image src={asset("/renders/garden-full.jpg")} alt="The same garden clearing filled with flowers after the event" width={1020} height={1080} priority style={{ width: "100%", height: "100%" }} />
      </motion.div>
    </div>
  );
}
